...
\ ESP32forth v7.0.5.4 does not support Structures, so we create these three structures by hand instead

16 constant RFparam                 \ room for the four 32 bit variables
...
create SCRATCHdata RFparam allot	\ create workspace so we can compare old and new data
                                    \ and thus determine which changed bytes need sendi

\ now the address offsets for the variables with the structures

: .OMD ;                            \ The first variable OMD is offset zero, so do nothing
: .msx_p1 4 + ;     		        \ OMD intermediate value + rx_div + msx_divby4
: .msna_p2 8 + ;    		        \ FMD intermediate value
: .msna_p1 12 + ;   		        \ FMD intermediate value 
