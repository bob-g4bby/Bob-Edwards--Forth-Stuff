( Patches to Polydos Nasforth v1.11 to increase code space )
...
BB00 12 +ORIGIN !   ( move the parameter stack S0 )
BBA0 14 +ORIGIN !   ( move the return stack R0 )
...
( more accurate FREE lists true code bytes free )
: FREE TIB HERE - U. ;  ( more accurate, excludes the buffers, stacks etc )

( then do a SYS-SAVE, <ctrl-shift-@> to exit to Polydos )
( and then create a BIGFORTH.GO file saving 1000 - C000 hex )
...
( The RAM disk is no longer support in Polydos Nasforth )
