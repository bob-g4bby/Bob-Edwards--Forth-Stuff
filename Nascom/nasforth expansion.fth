...
BBA0 10 +ORIGIN !   ( move the User Pointer UP ? SEE BELOW ) 
...
4BA0 BBA0 32  CMOVE ( copy the User Variables )
4BE0 BBE0 420 CMOVE ( copy the Disk Buffer )
...
BBA0 1022 !         ( UP is located at 1022 not 1010 - bug in manual? )

( more accurate FREE lists true code bytes available )
: FREE TIB @ HERE - U. ;  ( more accurate, excludes the buffers, stacks etc )


( this mimics the actions of SYS-SAVE )
LATEST 0C +ORIGIN ! ( update cold start - topmost word in dictionary )
HERE   1C +ORIGIN ! ( update cold start - FENCE initial value )
HERE   1E +ORIGIN ! ( update cold start - dictionary ptr init value )
HERE   FENCE !      ( update undeletable words pointer )
 
( then create a BIGFORTH.GO file saving 1000 - <HERE> hex, to start at 1000 hex )
